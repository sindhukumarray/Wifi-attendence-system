...
const FacultyPage = () => {
  const { loading, faculty, fetchFaculty } = useAdmin();
    ...
    <DashboardLayout sidebar={<Sidebar />} navbar={<Navbar />}>
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Faculty</h1>
            <p className="text-slate-500 font-medium mt-1">Manage teaching staff</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                  ...
                  {faculty.length > 0 ? (
                    faculty.map((member) => (
                      <tr key={member.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-slate-600 font-medium">{member.department_name || 'N/A'}</td>
                        <td className="px-6 py-4 font-mono font-bold text-slate-700">{member.sessions_count || 0}</td>
            ...
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/50">
              <p className="text-xs font-semibold text-slate-400">
                Total Faculty: <span className="text-slate-600 font-bold">{faculty.length}</span>
              </p>
            </div>
          </div>
        )}
